...
uint8_t bme280_init(uint8_t sensor){
    ...
    uint8_t test = bme280_read1Byte(BME280_REGISTER_CHIPID, sensor);
    printf("%d\n", test);
    switch (test){
        case 0x60:
        // BME280 connected
        ...
        TWI_MT_Start_SLAW(0xec|((1-sensor)<<1));
        TWI_MT_Send_Data(BME280_REGISTER_SOFTRESET);
        TWI_MT_Send_Data(0xB6);
        TWI_Stop();
        ...
        TWI_MT_Send_Data(BME280_REGISTER_CONTROLHUMID);
        TWI_MT_Send_Data(BME280_HUM_CONFIG);
        break;
        case 0x58:
        // BMP280 connected
        returnValue = 0x01;
    ...
    TWI_MT_Send_Data(BME280_REGISTER_CONFIG);
    TWI_MT_Send_Data(BME280_CONFIG);
    
    // write config for pressure, temperture and sensor-mode
    TWI_MT_Send_Data(BME280_REGISTER_CONTROL);
    TWI_MT_Send_Data((BME280_TEMP_CONFIG << 5)|(BME280_PRESS_CONFIG << 2)|(BME280_MODE_CONFIG));
...
uint8_t bme280_read1Byte(uint8_t addr, uint8_t sensor){
    uint8_t value;
    TWI_MT_Start_SLAW(0xec|((1-sensor)<<1));
    TWI_MT_Send_Data(addr);
    TWI_Stop();
    TWI_MT_Start_SLAW((0xec|((1-sensor)<<1))|0x01);
    value = TWI_readNAck();
    TWI_Stop();
    return value;
}
